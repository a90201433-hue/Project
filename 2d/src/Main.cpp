...
#include "Types.h"    // Здесь лежит NEQ - кол-во переменных в состоянии и field


int Nx_glob, Ny_glob; // ГЛОБАЛЬНОЕ число границ
int Nx, Ny;			  // ЛОКАЛЬНОЕ число границ в одном процессе

Domain dom; // Локальный характеристики доменчика
...
int main(int argc, char* argv[]) {
	...
	// 	if (dom.rank == r)
	// 	{
	// 		std::cout << "Rank " << dom.rank << std::endl;
	// 		for (int j = fict+1; j >= fict-1; j--)
	// 		{
	// 			for (int i = fict-1; i <= fict+1; i++)
	// 				std::cout << W[i][j][0] << " ";
	...
	while (step <= step_max && t <= t_max) {
		...
		// Глобальный минимальный dt
		MPI_Allreduce(MPI_IN_PLACE, &dt, 1,
					MPI_DOUBLE, MPI_MIN,
					MPI_COMM_WORLD);
		...
		if (step % step_fo == 0) {
			...
			for (int r = 0; r < p; r++) {
				...
				if (dom.rank == r) {
					...
					SaveFieldToCSV(W, dom.x, dom.y, t,
								   filename,
								   append);
				}
			}
