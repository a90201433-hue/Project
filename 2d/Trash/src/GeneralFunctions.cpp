...
void Euler(
	...
	DataArray F(Nx + 2 * fict);
	InitialZeros(F, 3);
    
	if (method == "TVD") {
		DataArray F_low(Nx + 2 * fict, {0.0, 0.0, 0.0});
		GetFluxes(W, F_low, "Godunov", TVD_solver, func, x, dt, Viscous_flag);

		DataArray F_high(Nx + 2 * fict, {0.0, 0.0, 0.0});
...
void RK3(
	...
	DataArray W1(Nx + 2 * fict - 1);
	InitialZeros(W1, 3);
	DataArray W2(Nx + 2 * fict - 1);
	InitialZeros(W2, 3);
	DataArray F(Nx + 2 * fict);
	InitialZeros(F, 3);
...
void TimePredictor(
	...
	DataArray F_left(Nx + 2 * fict); 
	InitialZeros(F_left, 3);
	DataArray F_right(Nx + 2 * fict); 
...
void FindBoundValues(std::vector<std::vector<double>> W, 
	...
	std::vector<std::vector<double>> Slope(Nx + 2 * fict, std::vector<double> (3, 0.0));
	std::vector<std::vector<double>> W_L(Nx + 2 * fict, std::vector<double> (3, 0.0));
	std::vector<std::vector<double>> W_R(Nx + 2 * fict, std::vector<double> (3, 0.0));
	std::vector<std::vector<double>> U(Nx + 2 * fict - 1, std::vector<double> (3, 0.0));
	std::vector<std::vector<double>> U_L(Nx + 2 * fict, std::vector<double> (3, 0.0));
	std::vector<std::vector<double>> U_R(Nx + 2 * fict, std::vector<double> (3, 0.0));
	std::vector<std::vector<double>> W_tilde(Nx + 2 * fict - 1, std::vector<double> (3, 0.0));
	std::vector<std::vector<double>> W_half(Nx + 2 * fict - 1, std::vector<double> (3, 0.0));
	std::vector<std::vector<double>> U_half(Nx + 2 * fict - 1, std::vector<double> (3, 0.0));
	...
	if (method == "Godunov") {
		...
		FindSlopes(W, Slope);
		ReconstructValues(W, Slope, W_L, W_R, func);
		TimePredictor(W_tilde, W, W_L, W_R, fict, Nx + fict - 1, x, dt);
		BoundCond(W_tilde);
...
void GetFluxes(
	...
	if (method == "WENO") {
		...
		std::vector<std::vector<double>> W_b(Nx + 2 * fict);
		InitialZeros(W_b, 3);
...
void NOperator(
	...
	std::vector<std::vector<double>> phi(Nx + 2 * fict);
	InitialZeros(phi, 3);
	std::vector<std::vector<double>> phi_c(Nx + 2 * fict);
	InitialZeros(phi_c, 3);
	std::vector<std::vector<double>> DU(Nx + 2 * fict - 1);
	InitialZeros(DU, 3);
	int s;
...
void UpdateArrays(
	...
	std::vector<std::vector<double>> W_L(Nx + 2 * fict);
	std::vector<std::vector<double>> W_R(Nx + 2 * fict);
	...
	if (time_method == "RK3") {
		RK3(W_new, W, method, solver, func, fict, Nx + fict - 1, x, dt, Viscous_flag);
	} else {
		Euler(W_new, W, method, high_order_method, solver, TVD_solver, phi, func, fict, Nx + fict - 1, x, dt, Viscous_flag);
	}
